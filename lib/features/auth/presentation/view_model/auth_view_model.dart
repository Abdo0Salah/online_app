...
class AuthViewModel extends Cubit<AuthStates> {
  ...
  void login (RequestLogin requestLogin) async {
    ...
    BaseResponse<ResponseLogin> response = await _loginUseCase.call(requestLogin);
    switch(response) {
      ...
      case ErrorResponse<ResponseLogin>():
        emit(state.copyWith(login:state.login.error(response.error.toString())));
    }
  }
  ...
  void verifyPassword (String resetCode) async {
    ...
    switch(response) {
      ...
      case ErrorResponse<VerifyResetCodeResponse>():
        emit(state.copyWith(verifyResetCodeResponse: state.verifyResetCodeResponse.error(response.error.toString())));
    }
  }

  void resetPassword ({required String email, required String newPassword}) async {
    emit(state.copyWith(resetPasswordResponse: state.resetPasswordResponse.loading));
    ...
    switch(response) {
      case SuccessResponse<ResetPasswordResponse>():
        emit(state.copyWith(resetPasswordResponse: state.resetPasswordResponse.loaded(response.data),));
      case ErrorResponse<ResetPasswordResponse>():
        emit(state.copyWith(resetPasswordResponse: state.resetPasswordResponse.error(response.error.toString())));
    }
  }
